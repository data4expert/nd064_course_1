...
# Global variable for connection count
...
# Function to get total count of connections to the database
...
def get_total_post_count():
    connection = get_db_connection()
    total_post_count = connection.cursor().execute('SELECT COUNT(*) FROM posts').fetchone()[0]
    connection.close()
...
def metrics():
    ...
    connection_count = get_total_connection_count()
    response = {
        'db connection_count': connection_count,
